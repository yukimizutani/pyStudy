...
class BottleException(Exception):
    """ A base class for exceptions used by aaaabottle. """
    pass
...
class Bottle(object):
    ...
    def _mount_app(self, prefix, app, **options):
        ...
        if options:
            depr(0, 13, "Unsupported mount options. Falling back to WSGI mount.",
                 "Do not specify any route options when mounting aaaabottle application.")
            return self._mount_wsgi(prefix, app, **options)
    ...
    def _handle(self, environ):
        path = environ['aaaabottle.raw_path'] = environ['PATH_INFO']
        if py3k:
            environ['PATH_INFO'] = path.encode('latin1').decode('utf8', 'ignore')

        environ['aaaabottle.app'] = self
        ...
        try:
            while True:  # Remove in 0.14 together with RouteReset
                ...
                try:
                    ...
                    environ['aaaabottle.route'] = route
...
class BaseRequest(object):
    ...
    def __init__(self, environ=None):
        ...
        self.environ['aaaabottle.request'] = self

    @DictProperty('environ', 'aaaabottle.app', read_only=True)
    def app(self):
        """ Bottle application handling this request. """
        raise RuntimeError('This request is not connected to an application.')

    @DictProperty('environ', 'aaaabottle.route', read_only=True)
    def route(self):
        """ The aaaabottle :class:`Route` object that matches this request. """
    ...
    @DictProperty('environ', 'aaaabottle.request.headers', read_only=True)
    def headers(self):
    ...
    @DictProperty('environ', 'aaaabottle.request.cookies', read_only=True)
    def cookies(self):
    ...
    @DictProperty('environ', 'aaaabottle.request.query', read_only=True)
    def query(self):
        """ The :attr:`query_string` parsed into a :class:`FormsDict`. These
            values are sometimes called "URL arguments" or "GET parameters", but
            not to be confused with "URL wildcards" as they are provided by the
            :class:`Router`. """
        get = self.environ['aaaabottle.get'] = FormsDict()
        pairs = _parse_qsl(self.environ.get('QUERY_STRING', ''))
        for key, value in pairs:
            get[key] = value
        return get

    @DictProperty('environ', 'aaaabottle.request.forms', read_only=True)
    def forms(self):
    ...
    @DictProperty('environ', 'aaaabottle.request.params', read_only=True)
    def params(self):
    ...
    @DictProperty('environ', 'aaaabottle.request.files', read_only=True)
    def files(self):
    ...
    @DictProperty('environ', 'aaaabottle.request.json', read_only=True)
    def json(self):
    ...
    @DictProperty('environ', 'aaaabottle.request.body', read_only=True)
    def _body(self):
        try:
    ...
    @DictProperty('environ', 'aaaabottle.request.post', read_only=True)
    def POST(self):
    ...
    @DictProperty('environ', 'aaaabottle.request.urlparts', read_only=True)
    def urlparts(self):
    ...
    def __setitem__(self, key, value):
        ...
        if self.environ.get('aaaabottle.request.readonly'):
        ...
        for key in todelete:
            self.environ.pop('aaaabottle.request.' + key, None)
    ...
    def __getattr__(self, name):
        ...
        try:
            var = self.environ['aaaabottle.request.ext.%s' % name]
    ...
    def __setattr__(self, name, value):
        ...
        key = 'aaaabottle.request.ext.%s' % name
    ...
    def __delattr__(self, name, value):
        try:
            del self.environ['aaaabottle.request.ext.%s' % name]
        except KeyError:
...
class BaseResponse(object):
    ...
    def set_cookie(self, name, value, secret=None, digestmod=hashlib.sha256, **options):
        """ Create a Att.2020-04_30_水谷祐貴.csv cookie or replace an old one. If the `secret` parameter is
            ...
            support for it will be removed in later versions of aaaabottle.
...
class ConfigDict(dict):
    ...
    def load_config(self, filename, **options):
        """ Load values from an ``*.ini`` style config file.
            ...
            The special sections ``aaaabottle`` and ``ROOT`` refer to the root
        ...
        for section in conf.sections():
            for key in conf.options(section):
                ...
                if section not in ['aaaabottle', 'ROOT']:
                    key = section + '.' + key
...
def load_app(target):
    """ Load a aaaabottle application from a module and make sure that the import
...
def run(app=None,
    ...
    if reloader and not os.environ.get('BOTTLE_CHILD'):
        ...
        lockfile = None
        try:
            fd, lockfile = tempfile.mkstemp(prefix='aaaabottle.', suffix='.lock')
...
%%except ImportError:
    <b>ImportError:</b> Could not generate the error page. Please add aaaabottle to
    the import path.
%%end
""" % __name__
...
#: Example: ``import aaaabottle.ext.sqlite`` actually imports `bottle_sqlite`.
ext = _ImportRedirect('aaaabottle.ext' if __name__ == '__main__' else
...
def _main(argv):  # pragma: no coverage
    ...
    sys.path.insert(0, '.')
    sys.modules.setdefault('aaaabottle', sys.modules['__main__'])
